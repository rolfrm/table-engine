// This file is auto generated by icy-table.
#ifndef TABLE_COMPILER_INDEX
#define TABLE_COMPILER_INDEX
#define array_element_size(array) sizeof(array[0])
#define array_count(array) (sizeof(array)/array_element_size(array))
#include "icydb.h"
#include <stdlib.h>
#endif


u64_f32_f32_vector_index * u64_f32_f32_vector_index_create(const char * optional_name){
  static const char * const column_names[] = {(char *)"index", (char *)"value"};
  static const char * const column_types[] = {"size_t", "f32_f32_vector_indexes"};
  u64_f32_f32_vector_index * instance = calloc(sizeof(u64_f32_f32_vector_index), 1);
  instance->column_names = (char **)column_names;
  instance->column_types = (char **)column_types;
  
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(size_t), sizeof(f32_f32_vector_indexes)}, (char *[]){(char *)"index", (char *)"value"});
  
  return instance;
}

void u64_f32_f32_vector_index_insert(u64_f32_f32_vector_index * table, size_t * index, f32_f32_vector_indexes * value, size_t count){
  void * array[] = {(void* )index, (void* )value};
  icy_table_inserts((icy_table *) table, array, count);
}

void u64_f32_f32_vector_index_set(u64_f32_f32_vector_index * table, size_t index, f32_f32_vector_indexes value){
  void * array[] = {(void* )&index, (void* )&value};
  icy_table_inserts((icy_table *) table, array, 1);
}

void u64_f32_f32_vector_index_lookup(u64_f32_f32_vector_index * table, size_t * keys, size_t * out_indexes, size_t count){
  icy_table_finds((icy_table *) table, keys, out_indexes, count);
}

void u64_f32_f32_vector_index_remove(u64_f32_f32_vector_index * table, size_t * keys, size_t key_count){
  size_t indexes[key_count];
  size_t index = 0;
  size_t cnt = 0;
  while(0 < (cnt = icy_table_iter((icy_table *) table, keys, key_count, NULL, indexes, array_count(indexes), &index))){
    icy_table_remove_indexes((icy_table *) table, indexes, cnt);
    index = 0;
  }
}

void u64_f32_f32_vector_index_clear(u64_f32_f32_vector_index * table){
  icy_table_clear((icy_table *) table);
}

void u64_f32_f32_vector_index_unset(u64_f32_f32_vector_index * table, size_t key){
  u64_f32_f32_vector_index_remove(table, &key, 1);
}

bool u64_f32_f32_vector_index_try_get(u64_f32_f32_vector_index * table, size_t * index, f32_f32_vector_indexes * value){
  void * array[] = {(void* )index, (void* )value};
  void * column_pointers[] = {(void *)table->index, (void *)table->value};
  size_t __index = 0;
  icy_table_finds((icy_table *) table, array[0], &__index, 1);
  if(__index == 0) return false;
  unsigned int sizes[] = {sizeof(size_t), sizeof(f32_f32_vector_indexes)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
      memcpy(array[i], column_pointers[i] + __index * sizes[i], sizes[i]); 
  }
  return true;
}

void u64_f32_f32_vector_index_print(u64_f32_f32_vector_index * table){
  icy_table_print((icy_table *) table);
}

size_t u64_f32_f32_vector_index_iter(u64_f32_f32_vector_index * table, size_t * keys, size_t keycnt, size_t * optional_keys_out, size_t * indexes, size_t cnt, size_t * iterator){
  return icy_table_iter((icy_table *) table, keys, keycnt, optional_keys_out, indexes, cnt, iterator);

}
